...
Assumption provenance is tracked in `papers/DGJ24OptimalStrategiesRCV/Assumptions.lean` and `papers/DGJ24OptimalStrategiesRCV/audit/assumption_match_llm.json` when assumption rows exist. No additional assumption-provenance table was separately recorded in this report refresh.
...
Current model-validator sidecars are the source of truth for timestamped rows. Model review has none recorded across 0 rows. Human dashboard review has 0/65 saved entries, with 0 stale rows and 0 human mismatches.
