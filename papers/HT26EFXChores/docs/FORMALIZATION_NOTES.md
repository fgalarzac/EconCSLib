...
- Paper: *EFX for Additive Chores: Nonexistence, Pareto Incompatibility, and Bi-Valued Existence*
- Authors: Wentao He and Biaoshuai Tao
- Version formalized: arXiv v2 (2026-07-09)
- Official URL: https://arxiv.org/abs/2606.08872v2
- Public PDF: https://arxiv.org/pdf/2606.08872v2.pdf
...
- Implementation theorem file: `HT26EFXChores/MainTheorems.lean`
- Human-facing theorem file: `HT26EFXChores/PaperInterface.lean`
- Machine-readable status source: `HT26EFXChores/status.json`
- Private outside-Lean proof plan: `HT26EFXChores/docs/FORMALIZATION_PLAN.md`
- Final validation report: `HT26EFXChores/FINAL_VALIDATION_REPORT.md`
- Dependency DAG: `HT26EFXChores/docs/DependencyDAG.tex`
- Rendered DAG: `HT26EFXChores/docs/DependencyDAG.pdf`
- LLM/source audit sidecars: `HT26EFXChores/audit/*.json`
...
`python3 scripts/review_dashboard.py --paper HT26EFXChores --statement-precheck`.
...
Then run `python3 scripts/review_dashboard.py --paper HT26EFXChores
...
`python3 scripts/review_dashboard.py --paper HT26EFXChores --precheck` only to
...
`python3 scripts/closeout_reuse_plan.py --paper HT26EFXChores` and execute only its
current `next_action`.
...
`python3 scripts/review_dashboard.py --paper HT26EFXChores --paper-coverage-precheck`
...
The strict repository closeout completed on 2026-08-17 with zero errors. The
rows below retain their proof-route detail and use that closeout status rather
than a pending receipt refresh.

| Paper item | Lean declaration | Status | File | Remaining assumptions / notes |
|---|---|---|---|---|
| Theorem 1 (tri-valued EFX nonexistence) (EFXadditivechores.tex:304-306) | `tri_valued_nonexistenceSpec` -> `tri_valued_nonexistence` | compiled proof; source-first audit completed | `PaperInterface.lean`; `AppendixTriArithmetic.lean`; `AppendixTriConstruction.lean`; `AppendixTriInstance.lean` | The explicit `n`-agent source construction, its two groupwise minima, and the final B/C pigeonhole contradiction are compiled. The transparent specification remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Four-agent construction proposition: at most one A item per bundle (EFXadditivechores.tex:340-342) | `tri_four_no_two_largeSpec` -> `tri_four_no_two_large` | compiled proof; source-first audit completed | `PaperInterface.lean`; `TriConstruction.lean` | The transparent `...Spec : Prop` remains the statement-audit target; the implementation is a finite counting proof, and source-first audit completed; strict closeout completed 2026-08-17 |
| Four-agent construction proposition: no-A bundle is expensive (EFXadditivechores.tex:363-365) | `tri_four_no_a_bundle_expensiveSpec` -> `tri_four_no_a_bundle_expensive` | compiled proof; source-first audit completed | `PaperInterface.lean`; `TriConstruction.lean` | The transparent `...Spec : Prop` remains the statement-audit target; the implementation is the source's singleton-and-counting argument, and source-first audit completed; strict closeout completed 2026-08-17 |
| Theorem 2 (EFX and Pareto-optimality incompatibility) (EFXadditivechores.tex:416-418) | `efx_pareto_incompatibilitySpec` -> `efx_pareto_incompatibility` | compiled proof; source-first audit completed | `PaperInterface.lean`; `ParetoConstruction.lean` | The explicit A/B/C instance, two EFX propositions, and low/high-focal Pareto improvements are compiled. The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Theorem 2 proposition: every EFX agent receives a large item (EFXadditivechores.tex:461-463) | `efx_po_every_agent_largeSpec` -> `efx_po_every_agent_large` | compiled proof; source-first audit completed | `PaperInterface.lean`; `ParetoConstruction.lean` | The proof formalizes both two-group and one-group source cases, including their B/C-swapped counterparts. The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Theorem 2 proposition: EFX cost lower bounds (EFXadditivechores.tex:502-504) | `efx_po_cost_lower_boundsSpec` -> `efx_po_cost_lower_bounds` | compiled proof; source-first audit completed | `PaperInterface.lean`; `ParetoConstruction.lean` | The proof uses the large-chore proposition, EFX capacity bounds, and the odd total number of chores. The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Theorem 3 (four-agent bi-valued EFX existence) (EFXadditivechores.tex:532-534) | `four_agent_bi_valued_efx_existsSpec` -> `four_agent_bi_valued_efx_exists` | compiled | `FullBiValuedDispatch.lean`; `B3OuterDispatch.lean` | All source branches, including the former B.4 outer dispatcher and public bridge, are compiled; source-first audit completed. |
| Lemma (M34 insertion) (EFXadditivechores.tex:577-580) | `m34_insertionSpec` -> `m34_insertion` | compiled proof; source-first audit completed | `PaperInterface.lean` | The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| B.2.1(b) disjoint maximum-type dispatch (EFXadditivechores.tex:2411-2443) | `existsEfxOfB1DisjointMaximum_dispatch` | compiled fixed-label source case | `B1DisjointPrefix.lean`; `B1DisjointSpecialAllocation.lean`; `B1DisjointResidualClassification.lean`; `B1DisjointDispatch.lean` | The canonical identity-or-swap prefix, both `(3,3)`/`(4,3)` direct schedules, and the ordinary nonexceptional/complementary-exceptional residual dispatcher are compiled. The outer maximum-type and graph selection are closed by the compiled B.1 dispatcher. |
| B.4 `b=3` graph/prefix dispatch (EFXadditivechores.tex:3006-3501) | `existsEfxOfM01M2_b3` | compiled | `B3OuterDispatch.lean` | The graph split, all B.4 subcases, relabelling, M34 extension, and final dispatcher are compiled. |
| Lemma (composition) (EFXadditivechores.tex:706-725) | `compositionSpec` -> `composition` | compiled proof; source-first audit completed | `PaperInterface.lean` | The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Lemma (canonical allocation properties) (EFXadditivechores.tex:760-770) | `canonical_allocation_propertiesSpec` -> `canonical_allocation_properties` | compiled proof; source-first audit completed | `PaperInterface.lean` | The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Lemma (balanced orientation) (EFXadditivechores.tex:1019-1034) | `balanced_orientationSpec` -> `balanced_orientation` | compiled proof; source-first audit completed | `PaperInterface.lean` | The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Lemma (EFX allocation of M2) (EFXadditivechores.tex:1045-1047) | `m2_efx_allocationSpec` -> `m2_efx_allocation` | compiled proof; source-first audit completed | `PaperInterface.lean` | The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Lemma (properties of the M2 EFX allocations) (EFXadditivechores.tex:1419-1462) | `m2_efx_allocation_propertiesSpec` -> `m2_efx_allocation_properties` | compiled proof; source-first audit completed | `PaperInterface.lean` | The transparent `...Spec : Prop` remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Lemma (exceptional-residue combination) (EFXadditivechores.tex:1709-1713) | `exceptional_residue_combinationSpec` -> `exceptional_residue_combination` | compiled proof; source-first audit completed | `ExceptionalCombination.lean`, `PaperInterface.lean` | The source phrase “canonical allocation of M01” is represented by the explicit small-for-at-most-one prefix bridge; source-first audit completed; strict closeout completed 2026-08-17 |
| Appendix A proposition: no bundle contains two A items (EFXadditivechores.tex:2074-2076) | `appendix_no_two_a_itemsSpec` -> `appendix_no_two_a_items` | compiled proof; source-first audit completed | `PaperInterface.lean`; `AppendixTriArithmetic.lean`; `AppendixTriConstruction.lean` | The proof follows the source's three-A, two-A-plus-another-item, and pure-two-A modular-contradiction branches. The transparent specification remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |
| Appendix A proposition: p1 and p2 lower bounds (EFXadditivechores.tex:2104-2106) | `appendix_p1_p2_lower_boundsSpec` -> `appendix_p1_p2_lower_bounds` | compiled proof; source-first audit completed | `PaperInterface.lean`; `AppendixTriArithmetic.lean`; `AppendixTriConstruction.lean` | The proof follows the source's A-free bundle, group-cardinality, and B/C-swapped counting arguments. The transparent specification remains the statement-audit target; source-first audit completed; strict closeout completed 2026-08-17 |

## Intake Checklist

## Shared-library provenance checkpoint

- Preferred upstream definition source: the distinct GameTheoryInLean
  [`EconCSLib`](https://github.com/gametheoryinlean/EconCSLib) snapshot at
  commit `cef01c7`.
- Reused/adapted material: its indivisible-allocation and owner-map design;
  the local chore module records that provenance adjacent to the adaptation.
- Near-miss audit: inspected upstream generic/2-agent EFX material did not
  supply a theorem proving this paper's EFX--Pareto incompatibility or its
  four-agent bi-valued EFX existence result. Those proofs remain paper-local.
...
- [ ] Confirm `scripts/audit_conclusion_provenance.py --paper HT26EFXChores` reports
...
- [ ] Update `status.json`, then run `python3 scripts/sync_paper_status.py
      --paper HT26EFXChores`. Defer the unscoped aggregate/site sync to integration
      or release.
...
- [ ] After the report/DAG/status updates, run
      `python3 scripts/closeout_reuse_plan.py --paper HT26EFXChores`, follow its
