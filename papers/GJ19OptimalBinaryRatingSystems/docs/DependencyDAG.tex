...
\node[dag_result] (T31Lex) at (24.8,0) {
  ...
  source cell rates, finite optimizer, and value-first/rate-second assembly
};
...
\node[dag_result] (T31) at (24.8,-3.3) {
  ...
  exact staged source certificate, with explicit first-stage tie convention
};
...
\node[dag_result] (C4) at (12.4,-6.6) {
  \textbf{Lemma C.4} \\
  finite selected pullbacks have positive rate; non-step source gaps have rate zero
};
...
\node[dag_result] (T32Cert) at (24.8,-9.9) {
  ...
  literal weighted runner, additive loss, and polynomial runtime
};
