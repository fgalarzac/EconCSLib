...
\node[dag_result, dag_template_legend] (legDone) at (4.2,0) {formalized\\finite result};
\node[dag_conditional, dag_template_legend] (legCert) at (8.4,0) {formalized\\certificate};
\node[dag_conditional, dag_template_legend] (legOpen) at (12.6,0) {source\\condition};
}
\daglegend{(legDef)(legDone)(legCert)(legOpen)}{Legend}
...
\node[dag_conditional] (C4Reverse) at (6.2,-11.6) {
...
\textbf{Scope note.} The green and orange boxes are the current Lean
  boundary. Orange boxes are proved certificate statements whose explicit
  grid, rate, weight, essential-infimum, local pointwise-rate, or iteration
  hypotheses remain visible. The reusable weighted Laplace skeleton behind
...
\draw[dag_dashed_arrow] (TC1.east) -- (C3C4.west);
\draw[dag_arrow] (C3Finite.north) -- (C3C4.south);
\draw[dag_dashed_arrow] (C3C4.east) -- (T31.west);
\draw[dag_arrow] (C3C4.west) to[out=180,in=105,looseness=1.1] (C4Reverse.north west);
\draw[dag_dashed_arrow] (C4Witness.east) -- (C4Reverse.west);
\draw[dag_dashed_arrow] (C4Reverse.east) -- (T31.south west);
\draw[dag_arrow] (FiniteRate.south) -- (T31Value.north);
\draw[dag_dashed_arrow] (T31Value.east) to[out=0,in=165,looseness=1.0] (T31.west);
\draw[dag_dashed_arrow] (FiniteRate.south east) to[out=-45,in=145,looseness=1.05] (T31.north west);
\draw[dag_dashed_arrow] (T31.south) -- (TB1.north);
\draw[dag_dashed_arrow] (C2C8.south) to[out=-90,in=110,looseness=1.05] (TB1.north west);
