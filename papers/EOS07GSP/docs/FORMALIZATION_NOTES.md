...
as source-facing rows. Theorem 8 includes the continuous full-history strategy,
actual conditional Bayes beliefs, arbitrary complete continuation deviations,
legal-history ex-post PBE existence, effective dropout-action uniqueness, and
the finite VCG-equivalent outcome.
...
| Theorem 8, generalized-English auction unique legal-history ex-post PBE with Bayes beliefs and VCG-equivalent outcome | `theorem8_dropout_formula_eq_bstar_threshold`, `theorem8_q_step1_dropping_after_q_review`, `theorem8_q_step2_waiting_before_q_review`, `theorem8_continuous_full_history_bayes_ex_post_review`; finite VCG support: `theorem8_source_event_strict_values_unique_pbe_formula_conclusion` | formalized | `PaperInterface.lean` | None; a below-clock threshold means immediate dropout and uniqueness is stated for the clock-clamped observable action. |
...
full-history legal-history ex-post PBE endpoint. Current statement, coverage,
source-record, review-surface, assumption, and source-proof-fidelity artifacts
live under `audit/`.

No theorem-facing assumption declaration is configured. Strict click-rate and
value-law conditions appear directly as visible source-model inputs, while
Bayes consistency, best-response behavior, uniqueness, and outcome claims are
constructed in Lean.
