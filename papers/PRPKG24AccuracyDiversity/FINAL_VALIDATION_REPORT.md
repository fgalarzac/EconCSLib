...
square-root-share conclusion is unchanged. The independent rank-varying
Bernoulli construction used for Theorem 2 replaces the internally inconsistent
``iid'' wording while retaining the displayed rank-dependent marginal schedule.
...
gives the concise mathematical readings for Proposition 2, Theorem 2,
Appendix D.1, Proposition 4, and Lemma D.5. The
[Theorem 2 model clarification](docs/THEOREM2_SOURCE_CLARIFICATION.md)
states the exact independent rank-varying Bernoulli model and its visible
nondegeneracy conditions. The [paper statement map](audit/paper_statement_map.json)
records the corresponding source anchors and row-level evidence.
