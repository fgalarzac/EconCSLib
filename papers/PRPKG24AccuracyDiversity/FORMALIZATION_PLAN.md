...
- Public arXiv TeX source checked: e-print `2307.15142`, with targeted
  source inspection for Theorem 1, Corollary 1, Lemma D.2, and Lemma D.4.
...
- Current build status: passes as of the 2026-06-04 planning refresh.
...
- Complete or effectively closed:
  - Example 1's all-consumed calibration warning, top-one log-relaxation
    calculation, and exact harmonic exponential top-one asymptotic calibration.
    The Lean statements prove the exact all-consumed all-romance
    optimum/unique-argmax consequence, the weighted-log relaxed optimum at
    `(p1*n, p2*n)`, and convergence of exact exponential top-one optima to
    calibrated shares `(p1,p2)` when `p1 + p2 = 1`.
  ...
  - Corollary 1's branch realizations:
    ...
    for any `γ > 1` with `α = γ/(γ-1)`. The source-facing route now also has
    concrete iid bounded reflected-power and iid Pareto order-statistic
    branches, and
    `paper_corollary1_any_nonnegative_gamma_concrete_model_sequence_formula`
    packages all four `γ` cases into one concrete model-existence theorem.
  - Proposition 4's final minimization step:
    `Proposition4AveragingCertificate` now captures and proves the source's
    final averaging/sup argument from a user-level log-value `rho`, an
    abstract averaging operator, the uniform average identity, an
    average-to-witness step, and domination by the large-`n` objective `Γ`.
    `paper_proposition4_averaging_uniform_minimizes` proves that the uniform
    profile minimizes `Γ` from this interface. Product-integrable kernel
    constructors now discharge the Fubini layer, and
    ...
    discharge the constant item-level user-integral identity from a transitive
    item action, measure-preserving user action, and diagonal kernel
    invariance. The older `Proposition4ContinuousSphereCertificate` remains as
    the endpoint needed after the analytic sphere/Laplace layer supplies the
    concrete profile space, uniform user measure, cosine-kernel action, anchor
    integral, and the uniform profile's `Γ` value/lower bound.
...
- Formalized with caveat:
  - Proposition 2: uniform top-`k` source route is closed asymptotically through
    a finite `(2T+1)/N` bound and now feeds Theorem 1(ii)'s `β = 1`,
    `γ = 1/2` bounded-support profile API. The paper's sharper `(T+1)/N`
    finite bound and the printed relaxed-optimizer formula are documented as a
    nonblocking source deviation.
- Partially formalized:
  - Theorem 1(i)-(iv) and Corollary 1: source-shaped certificate wrappers
    ...
    closed and exposes a scaled-marginal certificate, the exact reflected-power
    bounded order-statistic mean table now exposes a direct scaled-marginal
    certificate for every `β > 0`, the bounded Lemma D.2
...
- Not started or intentionally deferred:
  - Proposition 4's analytic sphere layer: the profile space, uniform sphere
    ...
    constant, and Laplace-principle reduction.  The final averaging/sup step
    is already isolated in `ContinuousSphere.lean`.
...
The immediate target is the continuous bounded source-distribution layer for
Theorem 1(ii), not another optimization wrapper. The current PRPKG build
already closes the finite-discrete branch under explicit source assumptions,
the exponential finite-sample top-`k` order-statistic branch, the bounded exact
power-marginal optimization checkpoint, Lemma D.2's bounded-tail analytic
finite-sum assembly, the concrete iid Pareto Theorem 1(iv) order-statistic
branch, and the conditional Proposition 4 endpoint. The missing bounded proof
is the bridge from actual sampled bounded laws to the `μ_D(i,a)` /
`TopKValueOracle` objects consumed by the closed optimization and asymptotic
layers.
...
The exact iid product-measure uniform order-statistic bridge is also closed:
`uniform01ProductMeasure_expectedUpperOrderStatistic_eq`,
`uniform01IidOrderStatisticMeanSeq`,
`uniform01_expectedOrderStatisticTopKSum_eq_uniformTopKValue`, and
`paper_theorem1_ii_uniform01_iid_order_statistic_sequence_homogeneity_of_paper_bound`
identify the actual iid `[0,1]` source with the closed-form uniform top-`k`
oracle and expose the Theorem 1(ii) `γ = 1/2` sequence/formula endpoints. Do
not redo the beta/binomial layer-cake calculation for this special law.
For general `β > 0`, the canonical reflected-power bounded order-statistic
mean table is now compiled as `boundedReflectedPowerOrderStatisticMean`.
`boundedReflectedPowerOrderStatistic_topK_loss_eq` identifies its endpoint
loss with a finite coefficient times
`boundedReflectedPowerCommonLossFactor`; the shared lemma
`EconCSLib.Math.gamma_ratio_nat_add_one_add_asymptoticEquivalent` proves the
positive-shift gamma-ratio asymptotic, and
`boundedReflectedPowerOrderStatistic_scaled_drop` proves the discrete
scaled-drop law internally. The paper-facing scaled-marginal checkpoint is
`paper_theorem1_ii_bounded_reflected_power_order_statistic_scaled_marginal_certificate`.
The concrete iid reflected-power source table is now identified with that
gamma-ratio table by
...
The paper-facing source endpoints are
...
Future bounded work should not reprove this table identification or
`scaled_drop`; it should target the arbitrary bounded-family bridge beyond the
closed uniform and reflected-power laws.
...
For source-facing bounded laws, a concrete distribution may now state the
near-endpoint assumption directly as `P[X >= M - x] ~ (c/β)x^β`; the helper
`BoundedTailCDFPowerSandwich.of_reflectedCDFMass_upper_endpoint_tail_sandwich`
packages that into the reflected-CDF convention, and
...
feeds it to the scaled-marginal interface.
If the law supplies an exact local upper-endpoint power identity, use
...
For the Pareto branch, the exact power-marginal optimizer route, concrete iid
source route, and direct Theorem 1(iv) sequence/formula endpoint are now all
closed. The reusable Gamma-ratio and finite-difference pieces live in
`EconCSLib.Foundations.Math.GammaAsymptotics`; PRPKG keeps paper-facing wrappers
for the Lemma D.4 constants and sequence. The concrete iid scale-one Pareto
source law is named by `paretoIidSampleMeasure`, is proved to be a probability
family by `paretoIidSampleMeasure_isProbabilityMeasure`, and is exposed as
`paretoIidOrderStatisticMeanSeq`. The cited-source identification
`paretoIidSampleMeasure_citedOrderStatisticSource` feeds
`paper_theorem1_iv_pareto_iid_order_statistic_scaled_marginal_certificate`, and
the strict diminishing-marginal wrapper is
`paper_lemmaD4_pareto_iid_order_statistic_topK_forward_marginal_strict_antitone_eventually`.
The new final optimization bridge is
`paretoIidOrderStatistic_largeGapCertificate`: it uses Wendel-style
Gamma-ratio bounds to compare the common marginal factor
`Γ(q+1)/Γ(q+2-1/α)` against neighboring powers and absorbs one-count shifts
with `invSqrtSuccError`. Paper-facing direct endpoints are
...
`theorem1_iv_pareto_iid_order_statistic_sequence`, and
`theorem1_iv_pareto_iid_order_statistic_formula`. No caller now supplies a
Pareto source, large-gap, or FOC certificate for the concrete iid branch.

The one-dimensional Pareto inputs live in the shared library:
`EconCSLib.Probability.Pareto.iidProductMeasure` gives the finite iid sample
measure, `EconCSLib.Probability.Pareto.paretoMeasure_one_real_Ici_eq` proves
the scale-one survival formula `P[X ≥ x] = x^(-α)` for `1 ≤ x`, and the
strict-tail/CDF and strict-threshold binomial wrappers support the
upper-order-statistic survival and tail-integral checkpoints. Source note: the
arXiv TeX source confirms that Theorem 1(iv) and the proof target immediately
before Lemma D.4 use the Pareto scale `B * a^(1/α)`. The later Lemma D.4 proof
line prints `B log a`; treat that as a source typo and keep the Lean target on
the Pareto power-law statement.

Corollary 1 is now closed as a concrete model-existence theorem. The parameter
algebra routes `γ = 0` through two-point Bernoulli, `0 < γ < 1` through the
bounded reflected-power iid source, `γ = 1` through exponential order
statistics, and `γ > 1` through the concrete iid Pareto order-statistic source.
The auxiliary certificate endpoint remains available, but the source-facing
corollary no longer depends on an external certificate.
...
3. In PRPKG, instantiate the remaining bounded-family source bridge:
   prove the source-density-to-reflected-CDF tail sandwich for
   ...
   `paper_lemma1_uniform01_iid_reflected_cdf_count_layer_top_k_loss_asymptotic`.
   The exact uniform top-`k` order-statistic closed form is also compiled
   ...
   and the concrete iid `[0,1]` product-measure order-statistic source is
   closed by
   `paper_theorem1_ii_uniform01_iid_order_statistic_scaled_marginal_certificate`
   and
   `paper_theorem1_ii_uniform01_iid_order_statistic_sequence_homogeneity_of_paper_bound`.
   For the reflected-power bounded mean table, use
   `boundedReflectedPowerSourceIidOrderStatisticMeanSeq_eq_boundedReflectedPowerOrderStatisticMean`
   and the `paper_theorem1_ii_bounded_reflected_power_source_iid_*` wrappers;
   that route already proves the source-table identification, gamma-ratio loss
   asymptotic, scaled-drop law, scaled-marginal certificate, and
   sequence/formula endpoints. For other bounded laws, prove the scaled-drop
   limit above and then use the compiled source wrappers ending in
   `_and_scaled_drop`; this is the sanctioned route to the bounded
   scaled-marginal certificate. The generic certificate constructor is
   `paper_theorem1_ii_bounded_order_statistic_scaled_marginal_certificate_of_loss_ae_and_scaled_drop`.
4. Do not redo Corollary 1 packaging: the concrete model-existence endpoint is
   closed. Proposition 4's concrete sphere/Laplace-defined endpoint is now
   instantiated in `ContinuousSphereConcrete.lean`; remaining pickup work there
   should focus only on the source-regularity/Gamma validation note.
   Treat Proposition 2's printed sharp constant as a documented nonblocking
   source deviation unless a future downstream result unexpectedly needs that
...
- Keep the continuous sphere Proposition 4 separate from the remaining bounded
  source-distribution route.
